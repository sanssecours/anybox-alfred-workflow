...
def throw_error():
    error_feedback = {
        "items": [{
            "title": (
                "It looks like Anybox it’s not running or haven’t installed."
            ),
            "subtitle": (
                "Press ⏎ to open Anybox or press ⌘ + ⏎ to install Anybox in"
                " Mac App Store."
            ),
            "arg": ["anybox://show"],
            "mods": {
...
def get_links():
    ...
    req = urllib.request.Request(
        "http://127.0.0.1:6391/search?" + data, headers=headers
    )
    try:
        with urllib.request.urlopen(req) as response:
            ...
            for link in list:
                ...
                item = {
                    ...
                    "arg": [url, link["id"]],
                    "icon": {"path": icon_relative_url + "/icon"},
                    ...
                    "mods": {
                        ...
                        "shift": {"valid": True, "arg": url, "subtitle": url},
                    },
                    "quicklookurl": url,
                }
                result.append(item)
...
links = get_links()
result = {"items": tags + folders + links}
